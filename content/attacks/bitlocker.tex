...
It then proceeds to access the BitLocker encrypted drive containing the \program{Windload.efi} \ac{OS} loader.
This all still happens during the \ac{UEFI} boot environment, before \code{ExitBootServices()} is called, as this is done within \program{Windload.efi}.
...
\code{OpenProtocol()} offers many additional options such as exclusivity and notifying consumers when a protocol is being uninstalled \cite[Section 7.3]{uefi-spec}.
...
The hook has to be implemented in memory, which remains loaded until the Windows Boot Manager uses \code{ReadKeyStrokeEx()}.
...
To make use of the recovery key we can use an open source software called \program{Dislocker}, which implements the \ac{FUSE} interface to offer mounting a BitLocker encrypted partitions under Linux, supporting read and write access \cite{dislocker}.

In \autoref{sec:windows:security:bde} we discussed how the BitLocker filter driver integrates into Windows.
To integrate \program{Dislocker} into \ac{UEFI}, we start by analyzing how the \ac{UEFI} \ac{NTFS} driver works.
...
The source code analysis shows that the file\-/wise access through \nameref{lst:simple-file-system-protocol} and the \nameref{lst:file-protocol}, that the driver offers, consists of multiple layers of abstractions on top of block\-/wise access to the underlying media through the \nameref{lst:block-io-protocol}.
...
This allows for our hooks to create a layer where we can perform \program{Dislocker} operations on blocks passing through.
A diagram of the volume access protocol stack with our \program{Dislocker} hook layer can be seen in \autoref{fig:dislocker-volume-access-protocol-stack}.
...
\begin{figure}[htb]%
    ...
    \caption{\program{Dislocker} Volume Access Protocol Stack}%
    \label{fig:dislocker-volume-access-protocol-stack}%
\end{figure}

When we look at the \program{Dislocker} source code, we find that it offers two main functions, \code{dislock()} and \code{enlock()}, each taking offset\-/length parameters, comparable to the \nameref{lst:disk-io-protocol} abstraction.
...
In our hooked \code{ReadBlocks} and \code{WriteBlocks} functions, we simply call \code{dislock()} and \code{enlock()} respectively and have the \program{Dislocker} implementation call the original \code{ReadBlocks} and \code{WriteBlocks} functions through \code{pread} and \code{pwrite} when it needs to access the underlying volume.
This way, \program{Dislocker} can choose which blocks to perform its operations on, as well as freely remap underlying sectors.
...
For the previous two attacks the timing of deploying the payload did not matter, as long as it was done before Windows loads the \program{HKLM\brackslash SOFTWARE} registry hive, thus simply performing the deployment as soon as the \ac{UEFI} payload is executed was an option, as this happens before any Windows boot related actions are performed.
With BitLocker, we have to deploy after \emph{BitLogger} was able to obtain the recovery key.
We then initialize \program{Dislocker} with the recovery key, to enable the transparent \nameref{lst:block-io-protocol} hook layer.
We then need to reconnect all \nameref{lst:driver-binding-protocol} instances to all controllers to trigger a (re-)evaluation.
The BitLocker encrypted drive now appears unencrypted to the \ac{NTFS} driver, allowing it to install its \nameref{lst:simple-file-system-protocol} instance on the encrypted device.
...
After doing this we need to disable the \program{Dislocker} hook layer again, as otherwise, Windows is unable to boot and instead attempts Windows recovery.
...
In our \code{ReadKeyStrokeEx()} hook, we can suppress the user's escape keystroke and return a different key to the Windows Boot Manager to prevent an environment switch.
...
The recovery prompt is not triggered, and we are unable to receive a recovery key and cannot initialize \program{Dislocker}.
...
The \ac{VMK} can then be simply passed to \program{Dislocker} for the initialization of our hook layer.