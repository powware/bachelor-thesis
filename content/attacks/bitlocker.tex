...
\lstinputlisting[language=C,caption={Example of using \lstinline{HandleProtocol} to retrieve an instance to the \hyperref[lst:simple-text-input-ex-protocol]{Simple Text Input Ex Protocol} to use \lstinline{ReadKeyStrokeEx} to read a pending key press},captionpos=b,label=lst:handle-protocol-simple-text-input-ex]{code/handle_protocol_simple_text_input_ex.c}
...
The function interfaces of \lstinline{HandleProtocol} and \lstinline{OpenProtocol} would allow for the return of allocated memory containing a copy of the protocol's content, but the implementors of drivers managing multiple devices are encouraged to keep track of private data, that is necessary to manage a device, but not part of the protocol interface. This private data struct contains the protocol instance, so that it is then possible to calculate the private data address using the protocol instance's address and the offset of the protocol within the struct \cite[8]{tianocore-edk2-driver-writer-s-guide}. This keeps the protocol interface limited to the public functionality. In \autoref{lst:private-data} we show an example of retrieving private data through the public protocol interface.

\lstinputlisting[language=C,caption={Example of using private data in the implementation of driver},captionpos=b,label=lst:private-data]{code/private_data.c}
