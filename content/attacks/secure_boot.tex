...
If we were to have overwritten the standard boot entry of the hard drive \program{EFI\textbackslash Boot\textbackslash bootx64.efi}, a copy of the Windows Boot Manager, Windows would now be rendered unbootable.

\subsection{Rootkit}

In \autoref{sec:uefi-pi:pi:security} we discussed how the \ac{PI} specification defines the usage of its two security architectural protocols, with them being required to be invoked on every call to \code{LoadImage()}, and that the Security2 Architectural Protocol is responsible for the implementation of Secure Boot authentication.
...
Now these two seem to be conflicting, but when we deploy our rootkit it is unaffected by Secure Boot and execute just like before.
