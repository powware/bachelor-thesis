...
We access the \ac{ESP} and create a copy of the Windows Boot Manager located under \program{EFI\brackslash Microsoft\brackslash Boot\brackslash bootmgfw.efi}.
...
We can now instruct the default mappings to be reset with the \mbox{\program{map -r}} command, to receive an updated list including the file systems now provided through the \ac{NTFS} driver.
...
As done before we now type the mapping name of the new file systems, to check the root directories' contents with \program{ls} until we find the partition containing the Windows installation and then enter \program{vol} to check the access rights.
...
The firmware of our \nameref{sec:test-setup:asrock} already ships with a read\-/only \ac{NTFS} driver included.
...
We can change the \ac{NTFS} driver to install its \nameref{lst:simple-file-system-protocol} under a \ac{GUID} different from the regular \code{gEfiSimpleFileSystemProtocolGuid}.
...
The Task Scheduler bookkeeps its active tasks in the Windows registry under the path \program{HKLM\brackslash SOFTWARE\brackslash Microsoft\brackslash Windows NT\brackslash CurrentVersion\brackslash Schedule\brackslash TaskCache}.
...
This \ac{GUID} references the task descriptors that are saved using task master keys, which are located under \program{HKLM\brackslash SOFTWARE\brackslash Microsoft\brackslash Windows NT\brackslash CurrentVersion\brackslash Schedule\brackslash TaskCache\brackslash Tasks} \cite[Section 10]{windows-internals-7-part2}.
A secondary copy of the task descriptors exists within the regular file system under \program{\%windir\%\brackslash system32\brackslash Task}, stored in the form of \acsu{XML} files.

We can use the Task Scheduler Configuration Tool to modify the \program{Proxy} task on a system under our control, changing the executable path as well as removing the delay configured in its trigger.
...
\lstinputlisting[language=bash,firstline=5,lastline=7,caption={Snippet of \program{whoami /all} invoked by the Proxy task},captionpos=b,label=lst:whoami]{code/whoami.txt}
...
For now, we place our payload manually in the Windows installation and then boot into Linux, where we can open the \program{HKEY\underbreak LOCAL\underbreak MACHINE/SOFTWARE} hive in \program{reged} and import our modified registry key.
...
Luckily the author of \program{reged} used distinct functions to access the hive file and the registry file, making it possible to create the \ac{UEFI} port and keep the original source code unmodified.
...
If we were to import a key into its exact path, as exported from our machine, it would not overwrite the victim's \program{Proxy} key.
...
The build process generates intermediary \program{.ffs} files, which are the sectioned files.

For our Windows payload, we can use a special \ac{EDK} II module type that takes binary files as input.
The generated result is a sectioned file of type \code{EFI\underbreak FV\underbreak FILETYPE\underbreak FREEFORM}.
This type puts no restrictions on the contained file sections \cite[Vol. 3, 2.1.4.1.7]{pi-spec}.
The output contains only one file section of type \code{EFI\underbreak SECTION\underbreak RAW} consisting of the binary payload.

Now that we have sectioned files corresponding to all our resources used in the attack we can import these into the target image with \program{UEFITool}.
