...
The installation process is identical for both options, we first locate and access the \ac{ESP} and create a copy of the Windows Boot Manager, that is located under \lstinline{EFI\\Microsoft\\Boot\\bootmgfw.efi}. We then replace the original with our bootkit as well as drop all our payload and other required files on the \ac{ESP}.
