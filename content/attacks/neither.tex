...
A call to \code{OpenVolume()} results in an instance of the \hyperref[lst:simple-file-system-protocol]{File Protocol} representing the root folder of the volume \cite[Section 13.4]{uefi-spec}.
...
For this we port the \program{reged} utility, so that our bootkit can use it, it is already written in \code{C} saving us a lot of work.
...
Luckily the author of \program{reged} used distinct functions to access the hive file and registry file, making it possible to create the \ac{UEFI} port, keeping the original source code unmodified.
...
This includes relaying the \code{LoadOptions} and \code{ParentHandle} of the \hyperref[lst:loaded-image-protocol]{Loaded Image Protocol} from the bootkit's image handle to the Windows Boot Manager's image handle.
...
Our \ac{UEFI} payload in the form of \program{.efi} executable images cannot be directly inserted with UEFITool, because of the \ac{FFS} file sectioning mentioned in \autoref{sec:ueif-pi:pi:pi-firmware-images}.
...
For our Windows payload we can use a special \ac{EDK} II module type which takes binary files as input, the generated result is a sectioned file of type \code{EFI\_FV\_FILETYPE\_FREEFORM}.
...
Now that we have \program{.ffs} files corresponding to all our resources used in the attack we can import these into the target image with UEFITool.
