...
For BitLocker and to fullfil Windows 11's general requirement that a \ac{TPM} 2.0 is present, we use \program{swtpm}.
...
The \ac{SPI} chip clip is connected to a programmer that can be managed with the Linux utility \program{flashrom}.

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\textwidth]{test-setup/chip-clip}
...
Additionally by dualbooting into Linux we can use \program{flashrom} to communicate with the \ac{SPI} chip directly for read and write access.
