...
Two copies of the Windows Boot Manager \program{bootmgfw.efi} are placed on the \ac{ESP}, one under \program{EFI\textbackslash Boot\textbackslash bootx64.efi} for the default boot behavior the installed hard drive and one under \program{EFI\textbackslash Microsoft\textbackslash Boot\textbackslash bootmgfw.efi} alongside boot resources such as the \ac{BCD}.
...
This is also the location of the final step of the Windows UEFI boot process, \program{Windload.efi}, the application repsonsible for loading the kernel into memory \cite[12. The Windows OS Loader]{windows-internals-7-part2}.
...
Depending on what hibernation status is set within the \ac{BCD} it may launch the \program{Winresume.efi} application, which reads the hibernation file and resumes kernel execution \cite[Section 12]{windows-internals-7-part2}.
...
Though some more advanced options may not be exposed and can be accessed through the \program{regedit.exe} application which provides a graphical user interface to travere and modify the Registry \cite[Section 10]{windows-internals-7-part2}.
...
There is no one to one mapping of registry root key to hive file, the \ac{BCD} file for example is also a hive file and is mapped into the Registry under \program{HKEY\_LOCAL\_MACHINE\textbackslash BCD00000000} \cite[Section 10]{windows-internals-7-part2}.
