...
Two copies of the \emph{Windows Boot Manager} \program{bootmgfw.efi} are placed on the \ac{ESP}, one under \program{EFI\textbackslash Boot\textbackslash bootx64.efi} for the default boot behavior (i.e., booting from the installed hard drive) and one under \program{EFI\textbackslash Microsoft\textbackslash Boot\textbackslash bootmgfw.efi} alongside boot resources such as the \ac{BCD}.
...
This is also the location of the final step of the Windows \ac{UEFI} boot process, \program{Windload.efi}, the application responsible for loading the kernel into memory \cite[12. The Windows OS Loader]{windows-internals-7-part2}.
...
A crucial part to the whole Windows ecosystem is the \emph{Registry}.
...
There is no one\-/to\-/one mapping of a registry root key to a hive file, the \ac{BCD} file, for example, is also a hive file and is mapped into the Registry under \program{HKEY\_LOCAL\_MACHINE\textbackslash BCD00000000} \cite[Section 10]{windows-internals-7-part2}.
