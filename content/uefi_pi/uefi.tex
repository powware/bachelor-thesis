...
On the \code{x64} \acsu{CPU} architecture the \ac{PF} hands over the execution in \code{64-bit} long mode which includes memory protection.
...
The \acsu{CPU} is operating in uniprocessor mode and a sufficient amount of stack is available \cite[Section 2.3.4]{uefi-spec}.
...
\begin{description}
    ...
    \item[Controller/Device handles] are interchangeably used to refer to physical and virtual devices that offer \ac{I/O} abstraction protocols.
        Physical device handles support the \nameref{lst:device-path-protocol} for generic path/location information \cite[Section 10.2]{uefi-spec}.
...
If the \hyperref[lst:device-path-protocol]{device path} of a boot option only points to a device instead of containing an additional \hyperref[lst:device-path-protocol]{file path} on that device, it attempts to load a default boot application with the \nameref{lst:simple-file-system-protocol} \cite[Section 3.1.2]{uefi-spec}.
For \code{x64} architecture this is \code{\brackslash EFI\brackslash BOOT\brackslash BOOTX64.EFI} \cite[Section 3.5]{uefi-spec}.
