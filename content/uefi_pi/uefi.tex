...
The firmware will try to connect device drivers to a controller by using the drivers instance of the \nameref{lst:driver-binding-protocol} and call the \code{Supported()} function on a controller handle.
...
If the device path of a boot option only points to a device instead to the file on a device, it attempts to load a default boot application with the \nameref{lst:simple-file-system-protocol} \cite[Section 3.1.2]{uefi-spec}. For x64 it uses the default path \code{\textbackslash EFI\textbackslash BOOT\textbackslash BOOTX64.EFI} \cite[Section 3.5]{uefi-spec}.
