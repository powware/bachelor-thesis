...
Presumable a user assumes to have encountered the recovery prompt through a false positive, a firmware update without prio BitLocker suspension, boot configuration related changes etc.
When there is no reasonable assumption for the user to have encounter the recovery prompt they should simply not enter the recovery key.
After all the inherent problem is that the system integrity has been violated, in what fashion is now unverifiable.
Leaving the system in a state where no further trust should not be put into the system.
A distinction about whether a false positive or malicious code has caused the recovery prompt is not possible.
It is also the only time a user has a chance to correctly react, as with their decision to further put trust the system, from hereinafter the malicious code will be part of the root of trust.

Windows allowing this security override sets a dangerous precedence with the recovery prompt, especially it does not display any warnings about inherent danger.
Instead of a recovery prompt the user should be made aware of the inherent loss of system integrity.
And any future adjustments in the recovery prompt to provide more user awareness can be manipulated by an attacker, as by definition the appearance of the prompt signals that there is no more system integrity.
It is already easily possible to modify the message and url of the recovery prompt as these can be configured through group policies \cite{microsoft-windows-bitlocker-group-policy-settings}, that are read form the \ac{BCD} hive \cite{microsoft-windows-bcd-settings-and-bitlocker}.
...
It is a last resort attack and relying on social engineering and is somewhat comparable to phishing.
